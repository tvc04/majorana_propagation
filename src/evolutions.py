...
def sim_lucj_and_ucj(natoms: int, rows: int, cols: int):
...
if __name__ == "__main__":
    ...
    max_bonds_rcs = sim_rcs(rows, cols)

    plt.semilogy(max_bonds_rcs, "--o")
    plt.axhline(2 ** (num_qubits / 2))

    plt.savefig("1_0.png")

    plt.clf()

    max_bonds_lucj, max_bonds_ucj, nqubits = sim_lucj_and_ucj(atoms, rows, cols)

    plt.plot(max_bonds_lucj, "--o", label="LUCJ")
    plt.plot(max_bonds_rcs, "--o", label="RCS")
    ...
    plt.semilogy(max_bonds_lucj, "--s", mec="black", alpha=0.5, label=f"LUCJ ({nqubits} qubits, {nlayers} layers)")
    plt.semilogy(max_bonds_ucj, "--s", mec="black", alpha=0.5, label=f"UCJ ({nqubits} qubits, {nlayers // 2} layers)")
    plt.semilogy(max_bonds_rcs, "--s", mec="black", alpha=0.5, label="RCS")
    plt.axhline(2 ** (num_qubits / 2), ls="--", color="black")
